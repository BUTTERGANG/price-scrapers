...
def _ensure_schema(conn) -> None:
    with conn.cursor() as cur:
        # Migrate legacy column types BEFORE the DDL below: the JSONB GIN /
        # expression indexes can only be created once extra_json is JSONB.
        _migrate_legacy_columns(cur)
        cur.execute(
            """
...
def _column_type(cur, table: str, column: str):
    cur.execute(
        """
...
def _migrate_legacy_columns(cur) -> None:
    """One-time migrations for columns created before the current schema:
    TEXT timestamps (ISO-8601 strings from the SQLite era) -> TIMESTAMPTZ,
    and extra_json TEXT -> JSONB (required by the GIN/expression indexes)."""
    for table, column in (
        ("prices", "scraped_at"),
        ("runs", "started_at"),
        ("runs", "finished_at"),
        ...
        if _column_type(cur, table, column) == "text":
    ...
    if _column_type(cur, "prices", "extra_json") == "text":
        logger.info("Migrating prices.extra_json TEXT -> JSONB")
        cur.execute(
            "ALTER TABLE prices ALTER COLUMN extra_json TYPE JSONB "
            "USING NULLIF(extra_json, '')::jsonb"
        )
